 ...
 * An abstract class with with an objective to manage conversion, and other checks checks on a unit ({@link SI}).<br><br>
 */
public class SI_manager {
    ...
    public static SI getUnit(String input) {
        ...
        if (hasValidName(input) || hasValidAbrev(input)) {
            ...
            throw new IllegalArgumentException("Invalid user input. \"" + input + "\" is not a valid Unit name or abbreviation.");
        }
    }
